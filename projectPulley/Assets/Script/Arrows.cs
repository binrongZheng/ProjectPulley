...
public class Arrows : MonoBehaviour {
	...
	void Update () {
		...
		for (int i = 0; i < arrowTensioForces.Length; i++) {
			...
			pulleyT [i] = Mathf.Clamp (pulleyT[i],0,0.2f);
		...
		vectorDir [1] = FinTensioForces[0].transform.position-new Vector3(FinTensioForces[0].transform.localScale.x/2,0,0)  - initPos [1].position;
